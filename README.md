...
Open the app in the browser and register a new viewer or analyst account if desired.
Sign in as the registered viewer to confirm summary-only access.
Sign in as a registered analyst to inspect records, filters, and read-only behavior.
Sign in as admin to create records, update data, and add users.
Refresh the page to confirm the new data persists.
Postman Collection
An importable Postman collection is included at postman_collection.json.
